...
class UserServiceTest {
    ...
    void setUp() {
        ...
        trueItemEntity.setName("Test");
        trueItemEntity.setPrice(1.0);
        trueItemEntity.setQuantity(1);

        falseItemEntity.setName("Nothing");
        falseItemEntity.setPrice(10.0);
        falseItemEntity.setQuantity(10);

        trueItemDto.setName(trueItemEntity.getName());
        trueItemDto.setPrice(trueItemEntity.getPrice());
        trueItemDto.setQuantity(trueItemEntity.getQuantity());

        falseItemDto.setName(falseItemEntity.getName());
    ...
    @Test
    void testFindAllItemsForUserShouldReturnListOfItemDtoWhenCalled() {
        // Given
        when(userRepository.findByEmail(trueUserDto.getEmail())).thenReturn(trueUserEntity);
        when(itemRepository.findByUser(trueUserEntity)).thenReturn(Arrays.asList(trueItemEntity, trueItemEntity));

        // When
        List<ItemDto> itemDtoList = userService.findAllItemsForUser(trueUserDto);

        // Then
        assertNotNull(itemDtoList, "The returned list should not be null.");
        assertEquals(2, itemDtoList.size(), "The returned list should contain two ItemDto objects.");

        List<String> itemNames = itemDtoList.stream().map(ItemDto::getName).toList();
        assertTrue(itemNames.containsAll(Arrays.asList("Test", "Test")), "The list should contain all item names.");
    }

    @Test
    void testSaveItemsForUserShouldDeleteExistingAndSaveNewItemsWhenCalled() {
        //Given
        List<ItemDto> itemDtoList = Arrays.asList(
                new ItemDto("Item 1", 10.0, 1),
                new ItemDto("Item 2", 15.5, 2)
        );
        when(userRepository.findByEmail(trueUserDto.getEmail())).thenReturn(trueUserEntity);

        //When
        userService.saveItemsForUser(itemDtoList, trueUserDto);

        //Then
        verify(userRepository).findByEmail(trueUserDto.getEmail());
        verify(itemRepository).deleteAllByUser(trueUserEntity);
        verify(itemRepository, times(itemDtoList.size())).save(any(ItemEntity.class));
    }
    ...
    @Test
    void testConvertToItemDtoShouldConvertAnItemEntityToItemDtoWhenCalled(){
        //Given-When
        ItemDto resultDto = userService.convertToItemDto(trueItemEntity);
        //Then
        assertEquals(trueItemEntity.getName(), resultDto.getName(), "It should match");
        assertEquals(trueItemEntity.getPrice(), resultDto.getPrice(), "It should match");
        assertEquals(trueItemEntity.getQuantity(), resultDto.getQuantity(), "It should match");
    }
}